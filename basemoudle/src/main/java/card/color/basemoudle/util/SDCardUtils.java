...
public class SDCardUtils {
    ...
    //创建用于保存临时存储目录
    public static String getRootCacheDirPath(Context context, String filePathName) {
        String DefaultPath = "";
        if (context != null) {
            DefaultPath = context.getCacheDir().getAbsolutePath();
            // 如果有外部存储就用外部存储
            if (isSDCardEnable()) {
                DefaultPath = context.getExternalCacheDir().getAbsolutePath();
            }
        }
        // 然后再获得一个绝对目录替换为保存目录
